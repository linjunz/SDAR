function hatD = DiffNet2(hatSigmaX,hatSigmaY,n,lambda1)
%%
[~,p]=size(hatSigmaX);
%[n2,p]=size(yt);
n1=n;
...
while cond(hatSigmaX)> 500 
      hatSigmaX = hatSigmaX + 100*sqrt(log(p)/n1)*diag(ones(1,p)); 
end
while cond(hatSigmaY)> 500 
